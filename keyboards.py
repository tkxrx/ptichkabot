...
def main_keyboard() -> InlineKeyboardMarkup:
    buttons = [InlineKeyboardButton(text=str(el), callback_data="m" + str(i+1)) for i, el in enumerate(
        ['1 График работы пунктов приема «Птичка»',
         '2 Что можно сдать в пункт?',
         '3 Как правильно сдавать вторсырье?',
         '4 Есть ли вознаграждение за вторсырье?',
         '5 Какие мероприятия проводит «Птичка»?',
         '6 Хотите посотрудничать?',
         '7 Если не нашлось ответа на мой вопрос?'])]
